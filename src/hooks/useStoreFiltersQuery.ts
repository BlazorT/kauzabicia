...
import { STORE_FILTERS_BODY, StoreDetail } from "@/utils/types";
import { useMemo } from "react";
import { useFilterStores } from "./useMenu";
...
export const useStoreFiltersQuery = (searchQuery: string) => {
  ...
  const { data, isPending, isError } = useFilterStores(storeFiltersBody);

  const storeData = useMemo(() => data?.data ?? [], [data]) as StoreDetail[];
